...
export const fixtures: SideportFixtureSet = {
  system: {
    ...
    checkedAt: iso(now),
    checks: [
      { id: 'mutation-protection', status: 'pass', source: 'demo', checkedAt: iso(now), scope: 'system', affectedResources: [], reason: 'Authenticated mutations are protected.' },
      { id: 'anisette-headers', status: 'pass', source: 'demo', checkedAt: iso(now), scope: 'apple-signer', affectedResources: [], reason: 'Provisioned anisette headers are available.' },
      { id: 'signer-executable', status: 'pass', source: 'demo', checkedAt: iso(now), scope: 'apple-signer', affectedResources: [], reason: 'The signer executable passed its bounded check.' },
    ],
  ...
  devices: [
    {
      ...
      acceptedAt: { value: iso(addDays(now, -30)), source: 'demo' },
      trustState: 'trusted',
      usableForInstall: true,
      supportedForFirstInstall: false,
      health: 'warning',
      teamId: 'M62Z4M5EUY',
      appSlotsUsed: 2,
      installedAppCount: 3,
    ...
    },
    {
      ...
      inventoryState: 'accepted',
      acceptedAt: { value: iso(addDays(now, -7)), source: 'demo' },
      trustState: 'trusted',
      usableForInstall: true,
  ...
  catalogApps: [
    {
      ...
      catalogVersion: 1,
      artifactSources: [{ kind: 'server', label: 'Sideport app storage' }, { kind: 'github-release', label: 'GitHub release', repository: 'dragoshont/sideport', releaseTag: 'sample-apps', assetName: 'Cert-Clock.ipa' }],
      ...
      iconTone: 'blue',
      notes: [
        'Demo metadata mirrors the Cert Clock seed; live runtime reads the path-free V2 catalog.',
    ...
    },
    {
      id: 'dice-roll',
      name: 'Dice Roll',
      purpose: 'A tiny one-tap dice roller for testing installs.',
      expectedBundleId: 'com.example.diceroll',
      catalogVersion: 1,
      artifactSources: [{ kind: 'server', label: 'Sideport app storage' }, { kind: 'github-release', label: 'GitHub release', repository: 'dragoshont/sideport', releaseTag: 'sample-apps', assetName: 'Dice-Roll.ipa' }],
      versionLabel: '0.1.0 (build 1)',
      status: 'ready',
      statusLabel: 'IPA inspected',
      source: 'demo',
      iconTone: 'amber',
      notes: ['Inspected IPA in Sideport storage.'],
      sizeBytes: 15360,
      sha256: 'demo-dice-sha256-redacted',
      hasEmbeddedProfile: false,
      lastInspectedAt: iso(now),
    },
    {
      id: 'concentration',
      name: 'Concentration',
      purpose: 'A simple card-matching memory game.',
      expectedBundleId: 'com.example.concentration',
      catalogVersion: 1,
      artifactSources: [{ kind: 'github-release', label: 'GitHub release', repository: 'dragoshont/concentration-ios', releaseTag: 'latest', assetName: 'Concentration.ipa' }],
      versionLabel: '0.1.0',
      status: 'ready',
      statusLabel: 'IPA inspected',
      source: 'demo',
      iconTone: 'green',
      notes: ['Imported from a configured GitHub release.'],
      sizeBytes: 24576,
      sha256: 'demo-concentration-sha256-redacted',
      hasEmbeddedProfile: false,
      lastInspectedAt: iso(now),
    },
  ],
  ...
  apps: [
    ...
    },
    {
      ...
      lastSucceeded: false,
      lastError: 'Invalid signature / Code=85 reported by device diagnostics.',
  ...
  issues: [
    ...
    },
    {
      ...
      operationId: 'op_verify_01JZ22_FAKE',
      traceId: 'trace-not-reported',
      spanSummary: [
        { name: 'sideport.device.verify-installation', durationMs: 460, state: 'failed' },
      ],
      logSnippet: 'Device diagnostics reported Code=85. Resolved by zsign SHA256-only signing path.',
      source: 'demo',
    },
  ],
  activity: [
  ...
  workspace: {
    ...
    authDelegated: true,
    capabilities: {
      'apple.signer.manage': true,
      'devices.enroll': true,
      'catalog.import': true,
      'integrations.github.manage': true,
      'operations.run': true,
    ...
    members: [
      ...
      { id: 'u-admin', name: 'Lab Admin', email: 'admin@example.test', role: 'family', status: 'active', lastActiveAt: iso(subHours(now, 26)), source: 'demo' },
      { id: 'u-operator', name: 'On-call Operator', email: 'oncall@example.test', role: 'family', status: 'active', lastActiveAt: iso(subHours(now, 5)), source: 'demo' },
      { id: 'u-viewer', name: 'Auditor', email: 'audit@example.test', role: 'family', status: 'active', invitedAt: iso(subHours(now, 48)), source: 'demo' },
    ],
  },
}
...
export const blockedFixtures: SideportFixtureSet = {
  ...
  system: {
    ...fixtures.system,
    operational: false,
    checks: fixtures.system.checks.map((check) => check.id === 'anisette-headers'
      ? { ...check, status: 'fail' as const, reason: 'Provisioned anisette headers are unavailable.', nextAction: 'Restore the persistent anisette identity.' }
      : check),
    ready: {
