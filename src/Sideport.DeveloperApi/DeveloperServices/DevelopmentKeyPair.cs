...
/// signer can use. Before the request is sent, the provider may persist an
/// owner-only PKCS#8 recovery copy so an issued certificate is not orphaned if
/// Apple's response is lost.
///
...
internal sealed class DevelopmentKeyPair : IDisposable
{
    ...
    /// <summary>
    /// Restore a key that was durably staged before asking Apple to mint a
    /// certificate. Keeping the same key across an ambiguous network failure is
    /// what lets Sideport recover the certificate Apple may already have issued.
    /// </summary>
    public DevelopmentKeyPair(ReadOnlySpan<byte> pkcs8PrivateKey)
    {
        RSA rsa = RSA.Create();
        try
        {
            rsa.ImportPkcs8PrivateKey(pkcs8PrivateKey, out int bytesRead);
            if (bytesRead != pkcs8PrivateKey.Length)
                throw new CryptographicException("The staged development key contains trailing data.");
            _rsa = rsa;
        }
        catch
        {
    ...
    /// <summary>
    /// Export the private key for an owner-only recovery file written before
    /// the certificate request leaves this process.
    /// </summary>
    public byte[] ExportPkcs8PrivateKey() => _rsa.ExportPkcs8PrivateKey();
