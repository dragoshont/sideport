...
| Wi‑Fi device **discovery** (mDNS `_apple-mobdev2._tcp`) | **REWRITE** in .NET (`Makaretu.Dns`/Zeroconf) **or** keep **netmuxd** as a tiny sidecar | Open risk: Netimobiledevice speaks the usbmux socket + lockdown TCP, but it is **not confirmed** to do its own Bonjour discovery. Don't presume it replaces netmuxd's mDNS. Decide after a spike. |
